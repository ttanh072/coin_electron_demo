...
class TabbedBrowserWindow {
  constructor(options) {
    ...
    this.tabs.on('tab-created', function onTabCreated(tab) {
      ...
      tab.webContents.executeJavaScript('({...localStorage});', true)
      .then((localStorage) => {
        const obj = JSON.parse(localStorage.vuex)
        let newURL = null
        if (obj.wallets.active.name != '') {
          newURL = 'chrome-extension://' + webuiExtensionId + '/popup.html#/home'
        } else {
          newURL = 'chrome-extension://' + webuiExtensionId + '/popup.html#/create-wallet'
        }
        if (newURL) tab.webContents.loadURL(newURL)
      })
...
class Browser {
  ...
  async init() {
    ...
    const webuiExtension = await this.session.loadExtension(path.join(__dirname, 'wallet'))
    webuiExtensionId = webuiExtension.id

    const newTabUrl = path.join('chrome-extension://', webuiExtensionId, 'popup.html#')

    const createWallet = 'create-wallet'

    const win = this.createWindow({ initialUrl: newTabUrl + createWallet })
